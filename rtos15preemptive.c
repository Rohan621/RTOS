...
uint32_t getArgument(void){

}

void destroyProcessPreempt(_fn fn)
{
    uint8_t i,j;
...
bool createProcessPreempt(_fn fn, int priority, char *label){
    bool ok = false;
    uint8_t i = 0;
    bool found = false;
    ...
    if (taskCount < MAX_TASKS)
    {
      ...
      if (!found)
      {
        ...
        stack[i][243] = 11;
        stack[i][244] = 3;
        stack[i][245] = 0xFFFFFFF9;
        stack[i][246] = 0;
        stack[i][247] = 1;
        stack[i][248] = 2;
        stack[i][249] = 3;
        stack[i][250] = 12;
        stack[i][251] = (uint32_t) fn;
        stack[i][252] = (uint32_t) fn;
        stack[i][253] = 0x01000000;


        // REQUIRED: preload stack to look like the task had run before
        tcb[i].sp = &stack[i][236]; // REQUIRED: + offset as needed for the pre-loaded stack
...
void svcCallIsr()
{
    ...
    __asm("    ldr r0, [r0,#56]   ");
    __asm("    ldrb r0, [r0,#-2]   ");
    svcNumber = getsvcNumber();
    switch(svcNumber){
    case 1:
        NVIC_INT_CTRL_R |= NVIC_INT_CTRL_PEND_SV;
        break;
    case 2:
        tcb[taskCurrent].ticks = argument;
        tcb[taskCurrent].state = STATE_DELAYED;
        NVIC_INT_CTRL_R |= NVIC_INT_CTRL_PEND_SV;
        break;
    case 3:
    {
        pSemaphore = (struct semaphore*) argument;
    ...
    case 5:{
        ...
        createProcessPreempt(argument, priority, label);
        break;
    }
    ...
    {
        argument = (void*)argument;
        destroyProcessPreempt(argument);
    }
    default:
        break;
    }
...
//    ok &=  createProcess(idle2, 6, "idle2"); // Task 0
    // Add other processes
